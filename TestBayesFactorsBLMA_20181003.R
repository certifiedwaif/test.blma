...
if (TRUE)
{
	...
	time8 <- system.time({blma_result_ZS_gauss_legendre  <- blma(vy, mX, prior="zellner_siow_gauss_laguerre"); })[3]
	...
	a4 <- proc.time()[3]
	bas.bic =  bas.lm(vy~., data=as.data.frame(mX),prior="BIC",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b4 <- proc.time()[3]
	t4 <- b4-a4
	cat(t4,"\n")

	a5 <- proc.time()[3]
	bas.hg =  bas.lm(vy~.,data=as.data.frame(mX),prior="hyper-g",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b5 <- proc.time()[3]
	t5 <- b5-a5
	cat(t5,"\n")

	a6 <- proc.time()[3]
	bas.hgl =  bas.lm(vy~.,data=as.data.frame(mX),prior="hyper-g-laplace",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b6 <- proc.time()[3]
	t6 <- b6-a6
	cat(t6,"\n")

	a7 <- proc.time()[3]
	bas.hgn =  bas.lm(vy~.,data=as.data.frame(mX),prior="hyper-g-n",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b7 <- proc.time()[3]
	t7 <- b7-a7
	cat(t7,"\n")

	a8 <- proc.time()[3]
	bas.ZSN =  bas.lm(vy~.,data=as.data.frame(mX),prior="ZS-null",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b8 <- proc.time()[3]
	t8 <- b8-a8
	cat(t8,"\n")

	a9 <- proc.time()[3]
	bas.ZSF =  bas.lm(vy~.,data=as.data.frame(mX),prior="ZS-full",modelprior=uniform(), initprobs="uniform", method="deterministic")
	b9 <- proc.time()[3]
	t9 <- b9-a9
	cat(t9,"\n")
